...
if ( !function_exists( 'unicaevents_options_settings_theme_setup' ) ) {
	...
	function unicaevents_options_settings_theme_setup() {
		...
		"info_service_3" => array(
			"title" => esc_html__('API Keys', 'unicaevents'),
			"desc" => wp_kses_data( __('API Keys for some Web services', 'unicaevents') ),
			"type" => "info"),
		
		'api_google' => array(
			"title" => esc_html__('Google API Key', 'unicaevents'),
			"desc" => wp_kses_data( __("Insert Google API Key for browsers into the field above to generate Google Maps", 'unicaevents') ),
			"std" => "",
			"type" => "text"),
